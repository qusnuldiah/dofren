...
@section('header_title', 'Tambah Cabang')
@section('content')

<div class="max-w-3xl mx-auto">

    <div class="bg-white/80 backdrop-blur-md rounded-3xl p-6 md:p-8 shadow-sm border border-white/60">
        <h2 class="font-heading font-bold text-xl text-[#3D1A10] mb-6">Tambah Cabang Baru</h2>
        ...
        <form action="{{ route('admin.branches.store') }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="misal: DoFren Malang Kota" required
                ...
                <div>
                    ...
                    <input type="text" name="city" value="{{ old('city') }}" placeholder="Malang" required
                           class="w-full px-4 py-3 bg-gray-50/50 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF7A00] focus:border-transparent outline-none transition-colors">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-[#3D1A10] mb-1.5">No. Telepon <span class="text-red-400">*</span></label>
                    <input type="text" name="phone" value="{{ old('phone') }}" placeholder="0341-xxxxxx" required
                           class="w-full px-4 py-3 bg-gray-50/50 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF7A00] focus:border-transparent outline-none transition-colors">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="address" rows="2" required
                          class="w-full px-4 py-3 bg-gray-50/50 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF7A00] focus:border-transparent outline-none transition-colors resize-none">{{ old('address') }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-semibold text-[#3D1A10] mb-1.5">Jam Operasional</label>
                <input type="text" name="open_hours" value="{{ old('open_hours') }}" placeholder="07:00 - 22:00"
                       class="w-full px-4 py-3 bg-gray-50/50 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF7A00] focus:border-transparent outline-none transition-colors">
            </div>

            <div>
                <label class="block text-sm font-semibold text-[#3D1A10] mb-1.5">Link Google Maps</label>
                <input type="url" name="maps_embed" value="{{ old('maps_embed') }}" placeholder="https://maps.google.com/..."
                       class="w-full px-4 py-3 bg-gray-50/50 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF7A00] focus:border-transparent outline-none transition-colors">
            </div>

            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" checked class="w-4 h-4 rounded border-slate-300 text-[#FF7A00] focus:ring-[#FF7A00]">
