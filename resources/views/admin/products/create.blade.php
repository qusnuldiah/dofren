...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white/80 backdrop-blur-md rounded-3xl p-6 md:p-8 shadow-sm border border-white/60">
        <h2 class="font-heading font-bold text-xl text-[#3D1A10] mb-6">Tambah Produk Baru</h2>
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-4 py-3 bg-gray-50/50 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF7A00] focus:border-transparent outline-none transition-colors resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="flex gap-6 pt-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_available" value="1" checked class="w-4 h-4 rounded border-slate-300 text-[#FF7A00] focus:ring-[#FF7A00]">
                    <span class="text-sm font-medium text-slate-600">Produk Tersedia</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_bestseller" value="1" {{ old('is_bestseller') ? 'checked' : '' }} class="w-4 h-4 rounded border-slate-300 text-[#FF7A00] focus:ring-[#FF7A00]">
